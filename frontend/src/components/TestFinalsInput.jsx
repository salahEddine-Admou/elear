...
const TestFinalInput = ({Timer}) => {
    ...
    const [correctOption, setCorrectOption] = useState(null);  // null indique qu'aucune option n'a été sélectionnée initialement

    const handleOptionChange1 = (index) => {
        setCorrectOption(index);  // Enregistrer l'indice de l'option cochée
    };
    useEffect(() => {
        setEditedQuestionText(''); // Clear edited question text for new input
        setOptions([]); // Reset options for the new question
        setEditableQuestionId(1); // Set new question as editable
...
const jso = {
    ...
    "options": [],  // Options sera rempli par la boucle
    "correctOption": correctOption  // L'indice de la bonne réponse, assurez-vous qu'il est valide
};

// Boucle pour ajouter chaque option dans le tableau 'options'
...
console.log(jso)
    ...
    const handleAddQuestion = () => {
        ...
        const newQuestion = {
            id: newQuestionId,
            text: '', // Initialize empty text for the new question
            options: []
        };
    
        setQuestions([...questions, newQuestion]); // Append new question to the array
        setEditedQuestionText(''); // Clear edited question text for new input
        setOptions([]); // Reset options for the new question
        setEditableQuestionId(newQuestionId); // Set new question as editable
        setCorrectOption(null); // Reset selected correct option
    };
    
    const name = localStorage.getItem('selectedTrainingName');
return (
    <>
        {showTest ? (
            <div className='mx-auto flex flex-col'>
                ...
                <div className="py-2 px-12">  
                    ...
                    <div className='relative w-[45rem] p-4 mx-auto mt-10 bg-gray-100 border-2 border-gray-100 focus-within:ring-1 focus-within:ring-blue-500'>
                        <p>You have  to complete the test</p>
    ...
    editableQuestionId === question.id && (
        <>
            <div className="relative w-[19rem] bg-gray-200 border-2 border-gray-500 focus-within:ring-1 focus-within:ring-orange-500">
                <p className="text-center font-bold mb-2">Question {question.id}</p>
            </div>
            <div className="flex flex-col gap-1 ">
                <div key={question.id} className="relative w-[19rem] mx-auto mt-2 bg-gray-100 border-2 border-gray-100 focus-within:ring-1 focus-within:ring-orange-500 flex items-center mb-4">
                    {editableQuestionId === question.id ? (
                        <input
                            type="text"
                ...
                <button
                                        className="relative  mt-6 left-72 text-gray-400 bottom-[1rem] w-[10rem] bg-gray-200 border-2 border-gray-400 focus-within text-center font-bold mb-2 ml-8"
                                        onClick={handleAddOption}
                                    >
                                        Add option(+)
                                    </button> 
                        <div className="relative w-[50rem] px-8 py-8 bg-gray-200 border-12 border-gray-100 focus-within:ring-1 ">
                                    
                                    <ul className="text-left list-disc ml-12 -mt-4">
                                        {options.map((option, index) => (
                                            // <li key={index} className="mb-2">
                                                <label className="flex mt-3 items-center">
                                                    <input
                                                      checked={correctOption === index}
                                                      onChange={() => handleOptionChange1(index)}
                                                        type="checkbox"
                        ...
                        <button
                            className="mt-4 bg-orange-500  text-black font-bold py-2 px-4 rounded"
